...
def svm_loss_vectorized(
    ...
    correct_class_scores = scores[torch.arange(scores.shape[0]), y].unsqueeze(1)  # Shape: (N, 1)

    # Compute margins
    margins = scores - correct_class_scores + 1  # delta = 1
    margins[torch.arange(scores.shape[0]), y] = 0  # Do not consider correct classes
    ...
    # loss.                                                                     #
    #############################################################################
    # Create a binary matrix of margins > 0
    margins_binary = margins_clipped.clone()
    margins_binary[margins_binary > 0] = 1

    # Count how many times we need to subtract for correct classes
    row_sum = margins_binary.sum(dim=1)
    margins_binary[torch.arange(scores.shape[0]), y] = -row_sum

    # Compute gradient
    dW = X.t().matmul(margins_binary) / X.shape[0]
    dW += 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def svm_get_search_params():
    ...
    Total combinations should be less than 25.
    """
    # Reduce the number of combinations to below 25
    learning_rates = [1e-4, 1e-3, 1e-2]  # 3 candidates
    regularization_strengths = [0.1, 1, 10]  # 3 candidates
...
def test_one_param_set(
    ...
    Returns:
    - cls (LinearClassifier): a trained LinearClassifier instance.
...
def softmax_loss_vectorized(
    ...
    # Compute gradient
    probs[torch.arange(num_train), y] -= 1  # dL/dscores
    dW = X.t().matmul(probs) / num_train
    dW += 2 * reg * W
    #############################################################################
    #                          END OF YOUR CODE                                 #
    #############################################################################
...
def softmax_get_search_params():
    """
    Return more granular hyperparameters for the Softmax model to improve cross-validation accuracy.
    """
    learning_rates = [1e-4, 5e-4, 1e-3, 5e-3, 1e-2]  # 5개
    regularization_strengths = [0.01, 0.1, 1, 10]  # 4개
    return learning_rates, regularization_strengths
