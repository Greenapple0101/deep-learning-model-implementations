...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    # Apply ReLU activation (manually, without torch.relu)
    hidden = hidden.clamp(min=0)  # ReLU activation
...
def nn_forward_backward(
    ...
    dscores /= N  # Average over batch size

    # Gradients for W2 and b2
    dW2 = h1.t().mm(dscores)  # Shape: (H, N) x (N, C) -> (H, C)
    db2 = torch.sum(dscores, dim=0)  # Shape: (C,)

    # Backprop into hidden layer
    dhidden = dscores.mm(W2.t())  # Shape: (N, C) x (C, H) -> (N, H)
    dhidden[h1 <= 0] = 0  # Backprop through ReLU

    # Gradients for W1 and b1
    dW1 = X.t().mm(dhidden)  # Shape: (D, N) x (N, H) -> (D, H)
    db1 = torch.sum(dhidden, dim=0)  # Shape: (H,)
...
def nn_get_search_params():
    ...
    ###########################################################################
    # You can adjust the above lists or add more values to explore            #
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
        ...
        for hs in hidden_sizes:
            for reg in regularization_strengths:
                for lr_decay in learning_rate_decays:
                    ...
                    # If validation accuracy is higher, update best_net, best_stat, best_val_acc
                    if val_acc > best_val_acc:
                        best_val_acc = val_acc
                        best_net = net
                        best_stat = stats
                    # Optionally, print out progress
                    print(f'lr: {lr}, hs: {hs}, reg: {reg}, lr_decay: {lr_decay}, val_acc: {val_acc}')
