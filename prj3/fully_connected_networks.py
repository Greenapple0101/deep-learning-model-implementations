...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        - cache: (x, w, b)
        """
        # Reshape the input to (N, D)
        N = x.shape[0]
        D = w.shape[0]
        x_reshaped = x.view(N, D)  # (N, D)

        # Compute the output
        out = x_reshaped.mm(w) + b  # (N, M)
    ...
    def backward(dout, cache):
        ...
        # Compute gradients
        db = dout.sum(dim=0)  # (M,)
        dw = x_reshaped.t().mm(dout)  # (D, M)
        dx_reshaped = dout.mm(w.t())  # (N, D)
        dx = dx_reshaped.view(x.shape)  # (N, d1, ..., d_k)
...
class ReLU(object):
    ...
    def forward(x):
        ...
        - cache: x
        """
        # Apply ReLU activation without in-place operations
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dout1, dW2, db2 = Linear.backward(dscores, cache2)
        dW2 += 2 * self.reg * W2  # Regularization gradient

        # Backward through first layer
        dX, dW1, db1 = Linear_ReLU.backward(dout1, cache1)
        dW1 += 2 * self.reg * W1  # Regularization gradient
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, self.num_layers + 1):
            W = weight_scale * torch.randn(layer_dims[i-1], layer_dims[i], dtype=dtype, device=device)
            b = torch.zeros(layer_dims[i], dtype=dtype, device=device)
            self.params[f'W{i}'] = W
            self.params[f'b{i}'] = b
        #######################################################################
        #                         END OF YOUR CODE                            #
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            caches[f'layer{i}'] = cache

            if self.use_dropout:
                out, dropout_cache = Dropout.forward(out, self.dropout_param)
                dropout_caches[f'dropout{i}'] = dropout_cache

        # Last layer: Linear
        W_last = self.params[f'W{self.num_layers}']
        b_last = self.params[f'b{self.num_layers}']
        scores, cache = Linear.forward(out, W_last, b_last)
        caches[f'layer{self.num_layers}'] = cache
        ##################################################################
        #                      END OF YOUR CODE                         #
        ##################################################################
        ...
        # Backward pass
        # Last layer
        dout, dW, db = Linear.backward(dscores, caches[f'layer{self.num_layers}'])
        ...
        for i in range(self.num_layers -1, 0, -1):
            if self.use_dropout:
                dout = Dropout.backward(dout, dropout_caches[f'dropout{i}'])
            dout, dW, db = Linear_ReLU.backward(dout, caches[f'layer{i}'])
...
def create_solver_instance(data_dict, dtype, device):
    """
    Create a Solver instance that trains a TwoLayerNet to achieve at least
    50% accuracy on the validation set.

    Inputs:
    - data_dict: Dictionary containing 'X_train', 'y_train', 'X_val', 'y_val'.
    - dtype: torch data type, e.g., torch.float32 or torch.float64.
    - device: Device to perform computations on, e.g., 'cpu' or 'cuda'.

    Returns:
    - solver: An instance of the Solver class configured to train the model.
    """
    # Initialize the TwoLayerNet with appropriate parameters
    model = TwoLayerNet(
        input_dim=3*32*32,    # Example input dimension (e.g., CIFAR-10 images)
        hidden_dim=200,       # Hidden layer size
        num_classes=10,       # Number of classes
        weight_scale=1e-2,    # Weight initialization scale
    ...
    optim_config = {
        'learning_rate': 1e-2  # You might need to tune this
    }

    # Initialize the Solver with the model and data
    solver = Solver(
        model=model,
        data=data_dict,
        update_rule=sgd,          # Using vanilla SGD; can switch to others
...
def adam(w, dw, config=None):
    ...
    m_corrected = config['m'] / (1 - beta1 ** t)

    # Update biased second moment estimate
    config['v'] = beta2 * config['v'] + (1 - beta2) * (dw ** 2)
    v_corrected = config['v'] / (1 - beta2 ** t)

    # Compute step
    step = (learning_rate * m_corrected) / (torch.sqrt(v_corrected) + epsilon)
    next_w = w - step
    #########################################################################
    #                              END OF YOUR CODE                         #
    #########################################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            mask = (torch.rand_like(x) > p).type(x.dtype) / (1.0 - p)
            out = x * mask
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
